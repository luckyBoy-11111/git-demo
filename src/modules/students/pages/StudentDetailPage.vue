...
import { mockParents, mockStudents } from '../utils/mock-data'
...
const student = computed(() => mockStudents.find((item) => item.id === route.params.id) || mockStudents[0])
const parents = computed(() => mockParents.filter((item) => item.studentId === student.value.id))
